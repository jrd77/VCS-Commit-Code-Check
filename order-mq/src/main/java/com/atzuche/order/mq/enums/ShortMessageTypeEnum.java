...
public enum ShortMessageTypeEnum {
    ...
    EXEMPT_PREORDER_PAYRENT_CAR_DEPOSIT_2_RENTER("ExemptPreorderPayRentCarDeposit2Renter", "您对“$carBrandTxt$”(车牌$CarPlateNum$)的租车订单(订单号：$orderNo$)租车押金已支付完毕10100202"),
    NOTIFY_RENTER_TRANS_REQACCEPTED("NotifyRenterTransReqAccepted", "车主已接受您对“ $carBrandTxt$$carType$”(车牌$carPlateNum$)的租车订单，请在1小时内支付租车押金，否则订单将被取消。如果订单时间需要提前或延后，请进入APP订单详情点击“修改订单“，车主同意后即可生效。"),
    ...
    NOTIFY_RENTER_TRANS_REQACCEPTED_SPECIAL_TIME("NotifyRenterTransReqAccepted_special_time", "车主已接受您对“ $carBrandTxt$$carType$”(车牌$carPlateNum$)的租车订单，请在1小时内支付租车押金，否则订单将被取消。如果订单时间需要提前或延后，请进入APP订单详情点击“修改订单“，车主同意后即可生效。租期包含10.1-10.7期间的订单，违章查询周期延长至33天，因违章查询周期延长，违章押金使用预授权、信用减免、绑卡免押的订单，违章押金将在期间转为消费，待订单查询无违章后原路退还"),
    PAY_ILLEGAL_DEPOSIT_2_OWNERSERVICE("PayIllegalDeposit2OwnerService", "$realName$预订您“$carBrandTxt$”(车牌$CarPlateNum$)(订单号：$orderNo$)的违章押金支付成功 101002021"),
    PAY_ILLEGAL_DEPOSIT_2_RENTER("PayIllegalDeposit2Renter", "您预订“$carBrandTxt$”(车牌$CarPlateNum$）(订单号：$orderNo$)违章押金付费成功10100202"),
    PAY_ILLEGAL_DEPOSIT_CANCEL_OWNER("PayIllegalDepositCancelOwner", "因租客未能完成违章押金支付，您$CarPlateNum$的订单已取消，请不要将车辆交于租客"),
    PAY_ILLEGAL_DEPOSIT_CANCEL_OWNERNEWYEAR("PayIllegalDepositCancelOwnerNewYear", "$realName$，由于租客未在取车时间前支付违章押金，您的“$carBrandTxt$”(车牌$CarPlateNum$)的订单已经自动取消(订单号：$orderNo$，取车时间：$RentTime$)，请不要将车辆交于租客，根据违约金规则（春节订单条款），您将获得最多500元违约金补偿。10100202"),
    PAY_ILLEGAL_DEPOSIT_CANCEL_RENTER("PayIllegalDepositCancelRenter", "由于您未在规定时间内支付违章押金，订单已被取消，系统将按平台规则扣除您的违约金"),
    PAY_ILLEGAL_DEPOSIT_CANCEL_RENTERNEWYEAR("PayIllegalDepositCancelRenterNewYear", "$realName$，由于您未在取车时间前支付违章押金，您对“$carBrandTxt$”(车牌$CarPlateNum$)的订单已自动取消(订单号：$orderNo$，取车时间：$RentTime$)。根据违约金规则（春节订单条款），您需要支付最多500元违约金。如还需用车请您在App重新下单，如有疑问请致电客服：10100202"),
    ...
    PAY_RENT_CAR_ILLEGAL_DEPOSIT_CANCEL_OWNER("PayRentCarIllegalDepositCancelOwner", "$realName$，由于租客未在规定时间内支付租车押金及违章押金，您的“$carBrandTxt$”(车牌$CarPlateNum$)的订单已经自动取消，并且不会产生违约责任(订单号：$orderNo$，取车时间：$RentTime$)。10100202"),
    PAY_RENT_CAR_ILLEGAL_DEPOSIT_CANCEL_RENTER("PayRentCarIllegalDepositCancelRenter", "$realName$，由于您未在规定时间内支付租车押金，您对“$carBrandTxt$”(车牌$CarPlateNum$)的订单已自动取消(订单号：$orderNo$，取车时间：$RentTime$)。如您是一时疏忽且还需用车，建议联系车主沟通后，在App重新下单，之前的车辆可在已取消订单中找到。客服热线：10100202"),
    REMIND_PAY_ILLEGAL_DEPOSIT_2_RENTER("RemindPayIllegalDeposit2Renter", "您预订的“$carBrandTxt$”(车牌$CarPlateNum$）尚未支付违章押金，如在取车前仍未完成，订单将默认取消，可能会产生违约金10100202"),
    REMIND_PAY_ILLEGAL_DEPOSITOWNER("RemindPayIllegalDepositOwner", "$realName$，您的“$carBrandTxt$”(车牌$CarPlateNum$)的一笔订单还未完成违章押金支付(订单号：$orderNo$，取车时间：$RentTime$)。为了您的爱车安全，请不要将车辆交于租客。如果在取车时间前租客仍未完成违章押金支付，订单将按租客违约取消处理。10100202"),
