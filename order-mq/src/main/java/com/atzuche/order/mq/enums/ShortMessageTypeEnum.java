...
public enum ShortMessageTypeEnum {
    ...
    DISTRIBUTE_EXEMPT_PREORDER_AUTO_CANCELORDER_2_RENTER("DistributeExemptPreOrderAutoCancelOrder2Renter", "$Call$，由于您未在规定时间内支付租车押金，订单已被系统自动取消"),
    ...
    PAY_ILLEGAL_DEPOSIT_2_OWNERSERVICE("PayIllegalDeposit2OwnerService", "$Renter$预订您“$CarBrand$”(车牌$CarPlateNum$)(订单号：$orderNo$)的违章押金支付成功$NewMsg$ 101002021"),
    ...
    PAY_ILLEGAL_DEPOSIT_CANCEL_OWNERNEWYEAR("PayIllegalDepositCancelOwnerNewYear", "$Call$，由于租客未在取车时间前支付违章押金，您的“$CarBrand$”(车牌$CarPlateNum$)的订单已经自动取消(订单号：$orderNo$，取车时间：$RentTime$)，请不要将车辆交于租客，根据违约金规则（春节订单条款），您将获得最多500元违约金补偿。10100202"),
    PAY_ILLEGAL_DEPOSIT_CANCEL_RENTER("PayIllegalDepositCancelRenter", "由于您未在规定时间内支付违章押金，订单已被取消，系统将按平台规则扣除您的违约金"),
    PAY_ILLEGAL_DEPOSIT_CANCEL_RENTERNEWYEAR("PayIllegalDepositCancelRenterNewYear", "$Call$，由于您未在取车时间前支付违章押金，您对“$CarBrand$”(车牌$CarPlateNum$)的订单已自动取消(订单号：$orderNo$，取车时间：$RentTime$)。根据违约金规则（春节订单条款），您需要支付最多500元违约金。如还需用车请您在App重新下单，如有疑问请致电客服：10100202"),
    PAY_RENT_CAR_DEPOSIT_2_OWNER("PayRentCarDeposit2Owner", "$Renter$预订您“$CarBrand$”(车牌$CarPlateNum$)(订单号：$orderNo$)的租车押金支付成功。干净的车况有助于提示租客爱护车辆并获得租客好评。"),
    PAY_RENT_CAR_DEPOSIT_2_RENTER("PayRentCarDeposit2Renter", "你对“$CarBrand$”(车牌$CarPlateNum$)的租车订单(订单号:$orderNo$)租车押金已支付完毕10100202"),
    PAY_RENT_CAR_ILLEGAL_DEPOSIT_CANCEL_OWNER("PayRentCarIllegalDepositCancelOwner", "$Call$，由于租客未在规定时间内支付租车押金及违章押金，您的“$CarBrand$”(车牌$CarPlateNum$)的订单已经自动取消，并且不会产生违约责任(订单号：$orderNo$，取车时间：$RentTime$)。10100202"),
    PAY_RENT_CAR_ILLEGAL_DEPOSIT_CANCEL_RENTER("PayRentCarIllegalDepositCancelRenter", "$Call$，由于您未在规定时间内支付租车押金，您对“$CarBrand$”(车牌$CarPlateNum$)的订单已自动取消(订单号：$orderNo$，取车时间：$RentTime$)。如您是一时疏忽且还需用车，建议联系车主沟通后，在App重新下单，之前的车辆可在已取消订单中找到。客服热线：10100202"),
    ...
    REMIND_PAY_ILLEGAL_DEPOSITOWNER("RemindPayIllegalDepositOwner", "$Call$，您的“$CarBrand$”(车牌$CarPlateNum$)的一笔订单还未完成违章押金支付(订单号：$orderNo$，取车时间：$RentTime$)。为了您的爱车安全，请不要将车辆交于租客。如果在取车时间前租客仍未完成违章押金支付，订单将按租客违约取消处理。10100202"),
    REMIND_PAY_ILLEGAL_DEPOSITRENTER("CancelOrder2Renter", "您还未支付预定车辆的违章押金，请在$RentTime$前完成支付。否则该订单将被取消，并扣除您的违约费用"),
    REMIND_PAY_ILLEGAL_DEPOSITRENTERSRV("RemindPayIllegalDepositRenterSrv", "$Call$，您还未支付租用“$CarBrand$”(车牌$CarPlateNum$)订单的违章押金(订单号：$orderNo$，取车时间：$RentTime$)。如果在取车时间前$hours$小时未完成违章押金支付，系统将取消订单，并按您违约取消处理。10100202"),
    REMIND_PAY_RENT_CAR_ILLEGAL_DEPOSITOWNER("RemindPayRentCarIllegalDepositOwner", "$Call$，您的“$CarBrand$”(车牌$CarPlateNum$)的一笔订单还未完成租车押金及违章押金支付(订单号：$orderNo$，取车时间：$RentTime$)。为了您的爱车安全，请不要将车辆交于租客。如果在取车时间前租客仍未完成租车押金及违章押金支付，订单将按租客违约取消处理。10100202"),
    REMIND_PAY_RENT_CAR_ILLEGAL_DEPOSITRENTER("RemindPayRentCarIllegalDepositRenter", "$Call$，您还未支付租用“$CarBrand$”(车牌$CarPlateNum$)订单的租车押金及违章押金(订单号：$orderNo$，取车时间：$RentTime$)。为了交易能正常开始，请尽快完成押金支付。如果在取车时间前未完成租车押金及违章押金支付，订单将按您违约取消处理。10100202"),
