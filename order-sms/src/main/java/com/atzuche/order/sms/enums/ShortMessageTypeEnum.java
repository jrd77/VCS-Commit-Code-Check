...
public enum ShortMessageTypeEnum {
    ...
    REMIND_PAY_ILLEGAL_DEPOSITOWNER("RemindPayIllegalDepositOwner", "$realName$，您的“$carBrandTxt$”(车牌$CarPlateNum$)的一笔订单还未完成违章押金支付(订单号：$orderNo$，取车时间：$expRentTime$)。为了您的爱车安全，请不要将车辆交于租客。如果在取车时间前租客仍未完成违章押金支付，订单将按租客违约取消处理。10100202"),
    REMIND_PAY_ILLEGAL_DEPOSITRENTER("CancelOrder2Renter", "您还未支付预定车辆的违章押金，请在$expRentTime$前完成支付。否则该订单将被取消，并扣除您的违约费用"),
    REMIND_PAY_ILLEGAL_DEPOSITRENTERSRV("RemindPayIllegalDepositRenterSrv", "$realName$，您还未支付租用“$carBrandTxt$”(车牌$CarPlateNum$)订单的违章押金(订单号：$orderNo$，取车时间：$expRentTime$)。如果在取车时间前$hours$小时未完成违章押金支付，系统将取消订单，并按您违约取消处理。10100202"),
    REMIND_PAY_RENT_CAR_ILLEGAL_DEPOSITOWNER("RemindPayRentCarIllegalDepositOwner", "$realName$，您的“$carBrandTxt$”(车牌$CarPlateNum$)的一笔订单还未完成租车押金及违章押金支付(订单号：$orderNo$，取车时间：$expRentTime$)。为了您的爱车安全，请不要将车辆交于租客。如果在取车时间前租客仍未完成租车押金及违章押金支付，订单将按租客违约取消处理。10100202"),
    REMIND_PAY_RENT_CAR_ILLEGAL_DEPOSITRENTER("RemindPayRentCarIllegalDepositRenter", "$realName$，您还未支付租用“$carBrandTxt$”(车牌$CarPlateNum$)订单的租车押金及违章押金(订单号：$orderNo$，取车时间：$expRentTime$)。为了交易能正常开始，请尽快完成押金支付。如果在取车时间前未完成租车押金及违章押金支付，订单将按您违约取消处理。10100202"),
