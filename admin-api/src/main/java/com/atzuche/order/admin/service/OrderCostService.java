...
public class OrderCostService {
	...
	public OrderRenterCostResVO calculateRenterOrderCost(RenterCostReqVO renterCostReqVO) throws Exception{
        ...
        //是否可以购买驾乘无忧险，补充保障费和轮胎险
        isPurchaseInsurance(realVo,data,req);

        //租金费用  费用明细表renter_order_cost_detail
        putRenterOrderCostDetail(realVo,data);
        //平台保障费、补充保障服务费 （长租不需要这两个费用）
...
//				int needIncrementAmt = cashierPayService.getRentCostBufuNew(renterCostReqVO.getOrderNo(), orderEntity.getMemNoRenter());
    ...
    private void isPurchaseInsurance(OrderRenterCostResVO realVo, com.atzuche.order.commons.vo.res.OrderRenterCostResVO data,OrderCostReqVO req) {

        OrderStatusDTO orderStatusDTO = data.getOrderStatusDTO();
        if(orderStatusDTO == null || orderStatusDTO.getStatus() == null){
            logger.error("订单状态查询失败,订单状态为空 req={}", JSON.toJSONString(req));
            throw new OrderStatusNotFoundException();
        }
        if(orderStatusDTO.getStatus() >= OrderStatusEnum.TO_RETURN_CAR.getStatus() || orderStatusDTO.getStatus() == OrderStatusEnum.CLOSED.getStatus()){
            realVo.setIsDriverInsur(0);
            realVo.setIsTyreInsur(0);
            realVo.setIsSupperInsurance(0);
            return;
        }
        List<RenterOrderCostDetailResVO> renterOrderCostDetailList = data.getRenterOrderCostDetailList();
        Optional.ofNullable(renterOrderCostDetailList).orElseGet(ArrayList::new).stream().forEach(x->{
           if(RenterCashCodeEnum.TYRE_INSURE_TOTAL_PRICES.getCashNo().equals(x.getCostCode())){
               realVo.setIsTyreInsur(0);
           }else if(RenterCashCodeEnum.DRIVER_INSURE_TOTAL_PRICES.getCashNo().equals(x.getCostCode())){
               realVo.setIsDriverInsur(0);
           }else if(RenterCashCodeEnum.ABATEMENT_INSURE.getCashNo().equals(x.getCostCode())){
               realVo.setIsSupperInsurance(0);
           }
       });
    }
...
//		 String adjustAmt = "0"; //默认
        ...
        Optional.ofNullable(ownerOrderFineDeatails).orElseGet(ArrayList::new).forEach(x->{
        ...
        Optional.ofNullable(consoleOwnerOrderFineDeatails).orElseGet(ArrayList::new).forEach(x->{
        ...
        int ownerModifyAddrAmt = CostStatUtils.calConsoleOwnerFineByFineType(FineTypeCashCodeEnum.MODIFY_ADDRESS_FINE, consoleOwnerOrderFineDeatailDTOS);
        int consoleRenterAdvanceReturnCarFienAmt = CostStatUtils.calConsoleOwnerFineByFineType(FineTypeCashCodeEnum.MODIFY_ADVANCE, consoleOwnerOrderFineDeatailDTOS);
        int consoleRenterDelayReturnCarFienAmt = CostStatUtils.calConsoleOwnerFineByFineType(FineTypeCashCodeEnum.DELAY_FINE, consoleOwnerOrderFineDeatailDTOS);
        ownerFine += CostStatUtils.calConsoleOwnerFineByFineType(FineTypeCashCodeEnum.CANCEL_FINE, consoleOwnerOrderFineDeatailDTOS)
                   + ownerGetReturnCarFienAmt
                   + ownerModifyAddrAmt
                   + renterAdvanceReturnCarFienAmt+consoleRenterAdvanceReturnCarFienAmt
                   + renterDelayReturnCarFienAmt+consoleRenterDelayReturnCarFienAmt;
		 ...
		 //统计
		 incomeAmtInt = adjustAmtInt + addOilSrvAmtInt + ownerFine + rentAmtInt + beyondMileAmtInt + oilAmtInt;
		 ...
		 realVo.setFineAmt(String.valueOf(ownerFine));
		 //值被覆盖了。。。
