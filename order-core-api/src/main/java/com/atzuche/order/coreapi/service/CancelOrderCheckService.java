...
public class CancelOrderCheckService {
    ...
    public void checkCancelOrder(CancelOrderReqContext reqContext) {
        ...
        //订单状态校验
        checkStatusCancelOrder(cancelOrderReqDTO.getConsoleInvoke(), reqContext.getOrderStatusEntity(), renterOrderEntity);
    }
     ...
     * 订单状态校验
     *
     * @param isConsoleInvoke   是否管理后台
     * @param orderStatusEntity 订单状态信息
     * @param renterOrderEntity 租客订单信息
     */
    public void checkStatusCancelOrder(boolean isConsoleInvoke, OrderStatusEntity orderStatusEntity, RenterOrderEntity renterOrderEntity) {
        if (null != orderStatusEntity) {
            ...
            if (!isConsoleInvoke) {
                //进行中的订单不能取消
                if (Objects.nonNull(orderStatusEntity.getStatus()) && orderStatusEntity.getStatus() >= OrderStatusEnum.TO_RETURN_CAR.getStatus()
