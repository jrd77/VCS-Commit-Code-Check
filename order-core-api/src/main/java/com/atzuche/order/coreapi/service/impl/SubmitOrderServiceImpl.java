...
public class SubmitOrderServiceImpl implements SubmitOrderService {
    ...
    @Override
    public ResponseData submitOrder(SubmitReqDto submitReqDto) {
        //调用日志模块
...
//====================落库OrderContextDto=========================
    ...
    public MemberTotalInfo getRenterMemberInfo(SubmitReqDto submitReqDto) throws RenterMemberException {
        ...
        if(responseData == null || !ErrorCode.SUCCESS.getCode().equals(responseData.getResCode())){
            log.error("Feign 获取租客会员信息失败,submitReqDto={},responseData={}",JSON.toJSONString(submitReqDto),JSON.toJSONString(responseData),e);
            //TODO  日志记录
    ...
    public MemberTotalInfo getOwnerMemberInfo(SubmitReqDto submitReqDto) throws RenterMemberException {
        List<String> selectKey = Arrays.asList(
                MemberSelectKeyEnum.MEMBER_CORE_INFO.getKey(),
                MemberSelectKeyEnum.MEMBER_AUTH_INFO.getKey(),
                MemberSelectKeyEnum.MEMBER_BASE_INFO.getKey(),
        ...
        if(responseData == null || !ErrorCode.SUCCESS.getCode().equals(responseData.getResCode())){
            log.error("Feign 获取车主会员信息失败,submitReqDto={},responseData={}",JSON.toJSONString(submitReqDto),JSON.toJSONString(responseData),e);
            //TODO  日志记录
            throw new RenterMemberException(ErrorCode.FAILED,"获取会员信息失败");
        }
        //TODO  日志记录
