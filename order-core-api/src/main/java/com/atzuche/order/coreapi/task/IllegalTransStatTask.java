...
/**
 * IllegalTransStatTask
 *
 * @author shisong
 * @date 2020/1/2
 */
@Component
@JobHandler("illegalTransStatTask")
public class IllegalTransStatTask extends IJobHandler {
    ...
    public ReturnT<String> execute(String s) throws Exception {
        ...
        try {
            ...
            List<IllegalToDO> list = orderSearchRemoteService.violateSettleOrder();

            logger.info("查询 远程调用 结算前15分钟订单,查询结果 models:[{}]", list);
            XxlJobLogger.log("查询 远程调用 结算前15分钟订单,查询结果 models:" + list);

            if(CollectionUtils.isNotEmpty(list) && START_FLAG == 1){
